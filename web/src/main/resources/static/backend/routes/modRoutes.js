module.exports = function(app, db) {
  ...
  app.get("/moderators/:id", (req, res) => {
    const sql = `SELECT m.mod_id, m.mod_name, m.gender, m.date_of_birth, ac.email 
                 FROM Moderators m JOIN Accounts ac ON m.mod_id = ac.account_id 
                 WHERE m.mod_id = ? LIMIT 1`;
    db.query(sql, [req.params.id], (err, results) => {
      if (err) return res.status(500).send(err);
      if (results.length === 0) return res.status(404).json({ message: "Moderator not found." });
      res.json(results[0]);
    });
  });

  // UPDATE profile Moderator
  app.put("/moderators/:id", (req, res) => {
    const { name, gender, date_of_birth } = req.body;
    db.query("UPDATE Moderators SET mod_name=?, gender=?, date_of_birth=? WHERE mod_id=?",
  ...
  app.post("/threads", (req, res) => {
    ...
    db.query("INSERT INTO threads (title, category_id, created_by) VALUES (?, ?, ?)",
  ...
  app.delete("/threads/:id", (req, res) => {
    ...
    db.query("UPDATE threads SET is_deleted=1, deleted_by=?, deleted_at=NOW() WHERE thread_id=?",
  ...
  app.put("/messages/:id", (req, res) => {
    ...
    db.query("UPDATE messages SET content=? WHERE message_id=?", [content, req.params.id], (err) => {
        if (err) return res.status(500).send(err);
        db.query("INSERT INTO Modifying (mod_id, message_id, modify_date) VALUES (?, ?, NOW())",
  ...
  app.post("/ban", (req, res) => {
    ...
    db.query("UPDATE appusers SET is_banned=1 WHERE user_id=?", [user_id], (err) => {
      if (err) return res.status(500).send(err);
      db.query("INSERT INTO ban (mod_id, user_id, ban_date, reason) VALUES (?, ?, NOW(), ?)",
