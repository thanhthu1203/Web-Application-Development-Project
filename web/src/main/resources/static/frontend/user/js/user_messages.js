...
function populateMessages() {
 ...
 // 1. Lấy threadId từ thanh URL (nếu có)
 const urlParams = new URLSearchParams(window.location.search);
 const threadIdFromUrl = urlParams.get('threadId');


 // 2. Lọc các bài post chính (không phải comment, không bị xóa)
 let mainPosts = ALL_MESSAGES.filter(m => !m.parent_id && !m.is_deleted);


 // 3. Nếu người dùng đi vào từ 1 Thread cụ thể -> Lọc tiếp
 if (threadIdFromUrl) {
   mainPosts = mainPosts.filter(m => m.thread_id == threadIdFromUrl);


   // UX: Đổi tiêu đề trang thành tên Thread
   const threadObj = THREADS.find(t => t.thread_id == threadIdFromUrl);
   if (threadObj) {
     document.querySelector('.page-title').textContent = threadObj.title;
     document.querySelector('.page-desc').textContent = "Posts in this thread";
    
     // UX: Tự động chọn sẵn Thread này trong khung "Create New Post" ở cuối trang
     const threadSelect = document.getElementById('threadSelect');
     if (threadSelect) threadSelect.value = threadIdFromUrl;
   }
 } else {
   // Phục hồi tiêu đề nếu click trực tiếp vào nút Messages ở thanh Menu bên trái
   const titleEl = document.querySelector('.page-title');
   if (titleEl) titleEl.textContent = 'Posts';
   const descEl = document.querySelector('.page-desc');
   if (descEl) descEl.textContent = 'All posts in the forum';
 }
 ...
 if (mainPosts.length === 0) {
   container.innerHTML = `
     <div class="empty-state">
       <div class="empty-icon">📭</div>
       <div class="empty-text">No posts found. Be the first to post!</div>
     </div>`;
   return;
 }
...
function buildPostCard(msg) {
 ...
 if (postAvatar) {
   avatarEl.innerHTML = `<img src="${postAvatar}" alt="Avatar"
                              style="inline-size:100%;block-size:100%;object-fit:cover;border-radius:50%;">`;
 } else {
   avatarEl.textContent = getInitials(msg.author_name);
 }
