...
function updateReportIndicator() {
   ...
   // 1. Lọc ra các report đang chờ xử lý (Pending)
   // Lưu ý: Bạn hãy điều chỉnh 'status' và 'pending' cho khớp với tên thuộc tính trong object REPORTS của bạn nhé.
   const pendingReports = REPORTS.filter(report => report.status === 'pending');
  
   // Tìm dấu chấm đỏ hiện tại (nếu có)
   const existingIndicator = reportNav.querySelector('.report-indicator');


   if (pendingReports.length > 0) {
       // Nếu có report pending và chưa có dấu chấm đỏ thì mới tạo
       ...
       // 2. Nếu không có report pending nào, xóa dấu chấm đỏ đi
       if (existingIndicator) {
...
// ------------------------------------------------------------------
// PHẦN SEARCH (Lấy từ user_threads.js sang)
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
// CẬP NHẬT HÀM RENDER (Truyền tham số threads vào)
// ------------------------------------------------------------------
/*function populateThreads(threadsArray) {
   const container = document.getElementById('modThreadsList');
   container.innerHTML = '';
  
   // Nếu không truyền tham số, tự động lấy các thread active
...
function lockThread(tid, state) {
   fetch(`${API}/threads/${tid}/lock`, {
       ...
       showToast('Updated successfully');
       THREADS = await fetch(`${API}/threads`, { headers: getAuthHeaders() }).then(r => r.ok ? r.json() : []);
       performSearch(); // Thay vì gọi populateThreads(), ta gọi performSearch để giữ nguyên bộ lọc đang có
   });
}
...
function deleteThread(tid) {
   ...
   fetch(`${API}/threads/${tid}`, {
       ...
       THREADS = await fetch(`${API}/threads`, { headers: getAuthHeaders() }).then(r => r.ok ? r.json() : []);
       performSearch(); // Giữ nguyên bộ lọc sau khi xóa
   });
}
...
async function init() {
   ...
   // Render danh sách thread ban đầu
   populateThreads();
}
