...
function populateMessages() {
 ...
 // 1. Đọc ID từ URL (do file mod_threads truyền sang)
 const urlParams = new URLSearchParams(window.location.search);
 const threadIdFromUrl = urlParams.get('threadId');


 // 2. Lấy các post chính (không phải comment, không bị xóa)
 let mainPosts = ALL_MESSAGES.filter(m => !m.parent_id && !m.is_deleted);


 // 3. Lọc nếu có threadId
 if (threadIdFromUrl) {
   ...
   // Trả lại tiêu đề mặc định nếu click trực tiếp vào menu Messages
   const titleEl = document.querySelector('.page-title');
   if (titleEl) titleEl.textContent = 'Manage Messages';
 ...
 if (mainPosts.length === 0) {
   ...
   if (tplEmpty) {
       ...
       container.querySelector('.empty-icon').textContent = '📭';
       container.querySelector('.empty-text').textContent = 'No posts found in this thread.';
   } else {
